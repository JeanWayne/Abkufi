...
db = client['Papers']
minlength = 1
collection = db['hindawi_1481807503629']
tonkenz = db['tokenz_hindawi_1481807503629']
...
for z in range(0,int(count/10)):
    cursor = collection.find({"year": ""+year+""}).skip(skip+z*10).limit(10)
        ...
        for sentence in sentences:
            ...
            for token in tokenized_sent:
                if re.match("^[a-zA-Z0-9]*$", token):
                    Ltoken = token.lower()
                    if Ltoken in stopwords.words('english') or token in ['(', ')', '-', '.', ',', ';', ':'] or re.match(
                            '^[0-9\.,]*[0-9]$', token):
                        continue
                    if wn.morphy(Ltoken) == None:
                        delta_lemma = re.sub(r'[^\w\s]','',Ltoken)
                        Capz=""
                        isAuthor="false"
                        ...
                        for dogen in punct:
                            if dogen in token and len(token) > minlength:
                                containsPunctuation="true"
                            if not len(token) == len(delta_lemma):
                                containsPunctuation = "true"

                        isAlreadyIn = tonkenz.find_one({"Token":""+token+"","document":document['_id']})
                        #isAlreadyInBulk = bulk.find({"Token":""+token+"","document":document['_id']})

                        if isAlreadyIn is None:
                            #print(token)
                            bulk.insert({"Token": "" + token + "", "cap": "" + Capz + "",
                                               "containsNumbers": "" + containsNumbers + "",
                                               "containsPunctuation": "" + containsPunctuation + "",
                                               "isAuthor": ""+ isAuthor +"",
                                                "length":  + len(token), "document":document['_id'], "paperName":document['journalName'],
                                                "path2file":document['path2file']})
                            bulkSize=bulkSize+1
